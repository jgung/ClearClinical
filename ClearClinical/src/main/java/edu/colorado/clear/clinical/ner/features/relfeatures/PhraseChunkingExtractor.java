...
public class PhraseChunkingExtractor implements RelationFeaturesExtractor
{
	                             ...
	                             DisorderSpan arg2) throws AnalysisEngineProcessException
	{
		...
		isFirst = false;
		for (TerminalTreebankNode aHeadList : headList)
		{
			if (aHeadList.getBegin() > arg2.getEnd())
			{
				if (!isFirst)
				{
					features.add(new Feature("PhraseChunk_After_FirstHead", aHeadList.getNodeValue()));
					isFirst = true;
				} else
				{
					features.add(new Feature("PhraseChunk_After_SecondHead", aHeadList.getNodeValue()));
					break;
				}
			}
		}
